...
class Chain {
    ...
    static class ContinuousBlock {
        ...
        public boolean equals(final Object o) {
            ...
            if (blockLength != that.blockLength) {
                return false;
            }
            if (fromStart != that.fromStart) {
                return false;
            }
            if (toStart != that.toStart) {
                return false;
            }
    ...
    void validate() {
        ...
        if (fromLength > fromSequenceSize) {
            throw new PicardException("From chain length (" + fromLength
                    + ") < from sequence length (" + fromSequenceSize + ") for chain " + id);
        }
        if (toLength > toSequenceSize) {
            throw new PicardException("To chain length (" + toLength
                    + ") < to sequence length (" + toSequenceSize + ") for chain " + id);
        }
        if (fromSequenceName.isEmpty()) {
            throw new PicardException("Chain " + id + "has empty from sequence name.");
        }
        if (toSequenceName.isEmpty()) {
            throw new PicardException("Chain " + id + "has empty to sequence name.");
        }
        if (blockList.isEmpty()) {
            throw new PicardException("Chain " + id + " has empty block list.");
        }
    ...
    public boolean equals(final Object o) {
        ...
        if (fromChainEnd != chain.fromChainEnd) {
            return false;
        }
        if (fromChainStart != chain.fromChainStart) {
            return false;
        }
        if (fromSequenceSize != chain.fromSequenceSize) {
            return false;
        }
        if (id != chain.id) {
            return false;
        }
        if (Double.compare(chain.score, score) != 0) {
            return false;
        }
        if (toChainEnd != chain.toChainEnd) {
            return false;
        }
        if (toChainStart != chain.toChainStart) {
            return false;
        }
        if (toNegativeStrand != chain.toNegativeStrand) {
            return false;
        }
        if (toSequenceSize != chain.toSequenceSize) {
            return false;
        }
        ...
        if (interval != null ? !interval.equals(chain.interval) : chain.interval != null) {
            return false;
        }
