...
public class LiftOverTest {
    ...
    private static final File TEST_DATA_DIR = new File("test/");
    private static final File CHAIN_FILE = new File(TEST_DATA_DIR, "hg18ToHg17.over.chain");
    //private static final File CHAIN_FILE = new File(TEST_DATA_DIR, "hg18ToHg19.over.chain");

    public static void main(String[] args) {
        try {
            LiftOverTest liftOver = new LiftOverTest();
            liftOver.initLiftOver();
            liftOver.testWriteChain();
        } catch (Exception ex) {
            ex.printStackTrace();
        }
    }
